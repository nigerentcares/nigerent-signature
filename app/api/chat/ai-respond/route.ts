 ...
 *
 * AI-first concierge responder.
 * 1. Fetches member profile (name, tier, wallet balance, last 5 requests)
 * 2. Fetches curated restaurant list + active offers
 * 3. Sends context + member message to Claude (haiku for speed/cost)
 * 4. If Claude detects a booking → creates ConciergeRequest / DiningRequest
 * 5. Returns AI response + optional booking metadata
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    // ── Step 1: Fetch member context ──────────────────────────────────────────
    const [member, recentRequests, restaurants, offers, recentMessages] =
      await Promise.all([
        // Member profile + tier + wallet
        ...
        // Last 5 concierge + dining requests
        Promise.all([
    ...
    const [conciergeReqs, diningReqs] = recentRequests
...
${conciergeReqs
  .map(r => `- [${r.status}] ${r.category}: ${r.description ?? 'No details'} (${r.createdAt.toLocaleDateString()})`)
  .join('\n')}
${diningReqs
  .map(r => `- [${r.status}] Dining at ${r.restaurant.name}: ${r.partySize} guests on ${r.preferredDate.toLocaleDateString()} at ${r.preferredTime}`)
  .join('\n')}
...
${restaurants
  .map(r => `- ${r.name} (${r.cuisine}, ${r.area}, ${r.city}) — Price Level: ${'₦'.repeat(r.priceLevel)} | Member Benefit: ${r.memberBenefit ?? 'None'} | Ambiance: ${r.ambianceTags.join(', ') || 'N/A'} | Notes: ${r.reservationNotes ?? 'None'}`)
  .join('\n')}
`.trim()
...
${offers
  .map(o => `- ${o.title} at ${o.partner.name} (${o.category}, ${o.city}) — ${o.shortDesc} [Eligible: ${o.tierEligibility.join(', ')}]`)
  .join('\n')}
`.trim()
    ...
    // Deduplicate: if history already includes the current message as the last user msg, remove the duplicate
    if (
      conversationMessages.length >= 2 &&
      conversationMessages[conversationMessages.length - 1].content ===
        conversationMessages[conversationMessages.length - 2].content &&
      conversationMessages[conversationMessages.length - 1].role === 'user' &&
      conversationMessages[conversationMessages.length - 2].role === 'user'
    ) {
      conversationMessages.splice(conversationMessages.length - 2, 1)
    }
    ...
    // ── Step 4: Parse response + handle booking ─────────────────────────────
    ...
    if (booking) {
      ...
      if (!bookingResult) {
        ...
        const concReq = await prisma.conciergeRequest.create({
          data: {
            userId: user.id,
            category,
            description: (booking.description as string) ?? message.trim(),
            status: 'RECEIVED',
            priority: 'STANDARD',
          },
        })
      ...
      // (Using a generic notification the concierge portal will pick up on refresh)
      await prisma.notification.create({
        data: {
          userId: user.id,
          type: bookingResult.type === 'dining' ? 'DINING_UPDATE' : 'CONCIERGE_REQUEST',
          title: 'New AI-Assisted Request',
          body: `${member.name} (${tierName}) — ${(booking.description as string) ?? message.trim().slice(0, 100)}`,
          ctaUrl: '/concierge',
          metadata: {
    ...
    // ── Step 5: Save AI response as chat message ────────────────────────────
